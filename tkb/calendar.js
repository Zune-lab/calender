    ...
    reader.onload = async function(event) {
        try {
            ...
            if (earliestDate) {
                ...
                const { error: settingsErr } = await supabase.from('user_settings')
...
async function loadSubjectDetails(relatedSubjectIds) {
    ...
    data.forEach(item => {
        ...
        const innerContent = `
            <div class="item-content"><span class="text">${escapeHtml(item.content)}</span>${originHtml}</div>
        ...
        if (item.type === 'note' || item.type === 'notification') {
            ...
            if (isDeleted) {
                ...
                li.innerHTML = `
                    <div class="item-content"><span class="text" style="text-decoration: line-through; opacity: 0.5;">${escapeHtml(item.content)}</span>${originHtml}</div>
                    <div class="item-actions" style="opacity: 1;">
                        ${actionBtn}
...
window.loadExams = async function() {
    ...
    if (exams && exams.length) {
        ...
        if (staleIds.length) {
            ...
            supabase.from('exams').delete().in('id', staleIds).then(({ error: delErr }) => {
...
// =========================================
const PLANNER_NOTIF_LEAD_MIN = 5;      // báo trước bao nhiêu phút so với giờ bắt đầu
...
function plannerNotifStatusText() {
    ...
    if (Notification.permission === 'granted' && plannerNotifEnabled()) return 'Đang bật — báo trước 5 phút, kể cả khi tắt trình duyệt.';
    return 'Đang tắt.';
}
        ...
        if (this.checked) {
            ...
            try {
                ...
                if (perm === 'granted') {
                    ...
                    new Notification('Đã bật thông báo Kế Hoạch', { body: 'Mình sẽ báo trước 5 phút khi sắp tới giờ 1 việc bạn đặt, kể cả khi bạn tắt trình duyệt.' });
                } else {
                    this.checked = false;
...
function startPlannerNotifPolling() {
    ...
    plannerNotifTimer = setInterval(checkUpcomingPlannerNotifications, PLANNER_NOTIF_POLL_MS);
}
...
async function checkUpcomingPlannerNotifications() {
    ...
    data.forEach(b => {
        ...
        if (minsUntil <= PLANNER_NOTIF_LEAD_MIN && minsUntil >= -1 && !notified.has(key)) {
...
async function loadPlannerBlocks() {
    ...
    if (error) {
        ...
        (data || []).forEach(r => {
            ...
            plannerBlocksByDate[key].push({ id: r.id, title: r.title || '', start_min: r.start_min, end_min: r.end_min, color: r.color || PLANNER_COLORS[0] });
        });
    }
    renderPlannerBlocks();
...
function buildPlannerBlockEl(b, dateStr) {
    ...
    el.className = 'planner-block' + (durationMin <= 40 ? ' is-compact' : '');
    el.dataset.id = b.id;
    ...
    el.innerHTML = `
        ...
        <div class="planner-block-body">
            ...
            <span class="planner-block-title">${plannerEscapeText(b.title) || 'Việc chưa đặt tên'}</span>
        </div>
...
function openPlannerEditor(block, e) {
    ...
    const pad = 12;
    const popRect = inner.getBoundingClientRect();
    const popW = popRect.width || 250;
    const popH = popRect.height || 300;
    let x = (e && e.clientX) || (window.innerWidth / 2);
    let y = (e && e.clientY) || (window.innerHeight / 2);
    x = Math.max(pad, Math.min(window.innerWidth - popW - pad, x - popW / 2));
    y = Math.max(pad, Math.min(window.innerHeight - popH - pad, y - 20));
...
async function savePlannerEditor() {
    ...
    if (draft.id) {
        ...
        if (local) { local.title = title; local.color = color; local.start_min = start; local.end_min = end; }
        renderPlannerBlocks();
        const { error } = await supabase.from('daily_plans').update({ title, color, start_min: start, end_min: end, push_notified: false }).eq('id', draft.id);
        ...
        const { data, error } = await supabase.from('daily_plans').insert([{
            ...
            start_min: start, end_min: end, color
        }]).select().single();
        ...
        plannerBlocksByDate[draft.dateStr].push({ id: data.id, title: data.title, start_min: data.start_min, end_min: data.end_min, color: data.color });
        renderPlannerBlocks();
    }

...
function handleSession(session) {
    ...
    if (session) {
        ...
        // XỬ LÝ AVATAR HOẶC TÊN (BẢO TOÀN NÚT ĐĂNG XUẤT)
                const meta = currentUser.user_metadata;
                const emailSpan = document.getElementById('user-email-display');
                const profilePill = document.querySelector('.profile-pill');
                
                if (emailSpan && profilePill) {
                    // Xóa ảnh cũ nếu có (chống trùng lặp khi F5)
                    const oldImg = profilePill.querySelector('.custom-avatar-img');
                    if (oldImg) oldImg.remove();

                    if (meta && meta.avatar) {
                        // NẾU CÓ ẢNH: Tạo thẻ ảnh nhét vào cạnh Email, sau đó giấu Email đi
                        const img = document.createElement('img');
                        img.src = meta.avatar;
                        img.className = 'custom-avatar-img';
                        img.style.cssText = 'width:34px; height:34px; border-radius:50%; object-fit:cover; border: 2px solid #FF3366; cursor:pointer; margin-right: 10px;';
                        img.title = "Sửa Hồ Sơ";
                        img.onclick = () => window.location.href = '../profile/index.html';
                        
                        profilePill.insertBefore(img, emailSpan);
                        emailSpan.style.display = 'none'; // Ẩn chữ đi
                    } else {
                        // NẾU KHÔNG CÓ ẢNH: Hiện Tên hiển thị, hoặc Email (trừ khi user chọn ẩn email)
                        ...
                        emailSpan.title = "Sửa Hồ Sơ";
                        emailSpan.onclick = () => window.location.href = '../profile/index.html';
                    }
                }
        ...
        .then(async ({data, error}) => { // ĐÃ THÊM ASYNC
            if(data && data.semester_start_date) {
                semesterStartDate = new Date(data.semester_start_date + 'T00:00:00');
            }
            // Đợi TKB chạy xong để nạp xong bảng màu global rồi mới chạy Lịch Thi
            await loadTimetable(); 
            loadExams();
        });
...
window.maxSemesterWeek = 20; // Biến toàn cục lưu số tuần thực tế
...
window.switchTab = function(tabId, element, titleText) {
    ...
    setTimeout(() => {
        ...
        // --- THUẬT TOÁN AUTO ĐỔI CHỨC NĂNG NÚT UPLOAD ---
        ...
        if (importLabel && importText) {
            if (tabId === 'timetable') {
                ...
                // Vào tab Cài đặt thì ẩn luôn nút đi cho giao diện sạch sẽ
...
document.getElementById('excel-file').addEventListener('change', async function(e) {
    ...
    reader.onload = async function(event) {
        try {
            ...
            // TỪ ĐIỂN QUY ĐỔI GIỜ HỌC CHUẨN SGU
            ...
            for (let i = 0; i < jsonArray.length; i++) {
                ...
                // THUẬT TOÁN AUTO TÍNH GIỜ (Chuyển Tiết thành Giờ chuẩn)
                ...
                if (startTiet > 0 && endTiet >= startTiet) {
                    formattedData.push({
                        ...
                        group_id: groupId, 
                        lecturer: lecturer, 
                        exact_time: exactTime, // <--- Đã được dịch ra giờ cụ thể (VD: 07:00 -> 09:50)
                        weeks: weeks
                    });
                }
            }
            ...
            await loadTimetable(); // ĐÃ THÊM AWAIT Ở ĐÂY
            loadExams(); // Cập nhật lại màu cho Lịch Thi
            e.target.value = '';
...
async function loadTimetable() {
    ...
    // --- 1. THUẬT TOÁN TÍNH SỐ TUẦN & TẠO TUẦN ĐỆM ---
    let actualMaxW = 1;
    ...
    // --- 2. GIAO DIỆN CHÚC MỪNG KẾT THÚC LỊCH HỌC ---
    if (selectedWeekNum > actualMaxW) {
        wrapper.innerHTML = `
            <div style="height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; opacity: 0.95; text-align: center; padding: 20px; animation: fadeInSlide 0.5s ease-out forwards;">
                ...
                <h4 style="font-size: 1.65rem; font-weight: 700; color: var(--text-primary); margin-bottom: 12px; letter-spacing: -0.5px;">Hoàn thành chương trình học!</h4>
                <p style="font-size: 1.05rem; color: var(--text-secondary); max-width: 450px; line-height: 1.6;">Lịch học chính thức của bạn đã kết thúc. Hãy theo dõi mục <strong style="color: var(--accent-color);">Lịch Thi</strong> và dành thời gian ôn tập thật tốt nhé!</p>
            </div>
        `;
        return; 
    }

    // --- 3. VẼ LƯỚI TKB BÌNH THƯỜNG CHO CÁC TUẦN CÒN LẠI ---
    const matrix = {};
    for (let t = 1; t <= 14; t++) {
        matrix[t] = {};
        for (let d = 2; d <= 8; d++) {
    ...
    // Xuất bảng màu ra biến toàn cục để tab Lịch Thi có thể lấy lại chính xác 100%
    ...
    for (let t = 1; t <= 14; t++) {
        ...
        for (let d = 2; d <= 8; d++) {
            ...
            if (slot.status === 'start') {
                ...
                // Lưu lại màu chính xác của môn này (dù là user tự custom hay tự động)
...
window.loadExams = async function() {
    ...
    if (!exams || exams.length === 0) {
        wrapper.innerHTML = `
            <div style="height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; opacity: 0.6; min-height: 400px;">
                ...
                <h5 style="font-size: 1.2rem; font-weight: 500; margin-bottom: 8px; color: var(--text-primary);">Chưa có lịch thi nào</h5>
                <p style="font-size: 0.95rem; color: var(--text-secondary);">Nhấn nút <strong style="color:var(--text-primary);">"Nhập Lịch Thi" ở góc trên bên phải</strong> để đồng bộ dữ liệu.</p>
            </div>
        `;
        return;
    }
    ...
    // --- THUẬT TOÁN ĐỒNG BỘ MÀU TỪ LƯỚI TKB ---
    const gradientColors = [ ['#FF3366', '#FF9933'], ['#00C9FF', '#92FE9D'], ['#7c3aed', '#E100FF'], ['#11998e', '#38ef7d'], ['#FDBB2D', '#22C1C3'] ];
    
    const subjectColorMap = {}; 
    let colorIndex = 0;
    // Bắt buộc quét lại allLoadedSubjects để tái tạo đúng 100% thứ tự màu của TKB
    ...
    // Lấy màu từ Setting (ưu tiên cao nhất)
    let savedSubjectColors = {};
    try { savedSubjectColors = JSON.parse(localStorage.getItem('subjectCustomColors')) || {}; } catch(e){}
    ...
    exams.forEach(exam => {
        ...
        let c1 = '#FF3366'; // Màu dự phòng
        
        // Cấp bậc ưu tiên màu:
        // 1. Màu do người dùng tự Set trong Cài đặt
        // 2. Màu do TKB tự động gán lúc quét (nếu TKB đã load)
        // 3. Màu do thuật toán giả lập tự tính
...
// =========================================
// 10. TỰ ĐỘNG CHUYỂN TAB TỪ DASHBOARD
// =========================================
window.addEventListener('DOMContentLoaded', () => {
    // Đợi 0.5s cho DOM và Supabase nạp xong rồi mới lật tab
    setTimeout(() => {
        const target = localStorage.getItem('targetTab');
        if (target) {
            let titleMap = { 'timetable': 'Thời Khóa Biểu', 'exams': 'Lịch Thi', 'settings': 'Cài Đặt' };
            const tabs = document.querySelectorAll('.nav-links li');
            for(let li of tabs) {
                if(li.getAttribute('title') && li.getAttribute('title').includes(titleMap[target])) {
                    li.click(); // Giả lập hành động nhấp chuột của người dùng
                    break;
                }
            }
            localStorage.removeItem('targetTab'); // Dọn dẹp dấu vết
        }
    }, 500); 
});